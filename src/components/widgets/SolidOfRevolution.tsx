// Widget: solid-of-revolution
//
// A schematic of a solid generated by revolving a plane region about an axis.
// Three representative elements are supported:
//   - disk   : revolve the region under y = f(x) about the x-axis; the element
//              is a vertical ellipse (the disk seen edge-on) of radius R = f(x),
//              contributing dV = pi R^2 dx.  (2D cross-section view)
//   - washer : same, but with an inner boundary r = g(x); the element is an
//              ellipse with a hole, contributing dV = pi (R^2 - r^2) dx.
//   - shell  : revolve the region about the y-axis. This case is drawn as a
//              FAUX-3D nest of concentric cylindrical shells (oblique/cavalier
//              projection): each shell is a cylinder of radius x and height
//              f(x), and the draggable representative shell is highlighted as a
//              distinct 3D tube of thickness dx, contributing dV = 2*pi*x*f(x)dx.
//
// The representative element is draggable along [a, b]; the readout shows the
// element's volume term and the total volume (numeric integral).
...
// Faux-3D shell scene.
//
// The shells share one vertical revolution axis (the y-axis). We use a simple
// oblique projection: a horizontal circle of data-radius r at data-height h maps
// to an ELLIPSE centred on the axis whose horizontal semi-axis is r*sx and whose
// vertical semi-axis is r*sx*tilt (the foreshortening of looking slightly down at
// the rim). Increasing height moves the ellipse straight up the screen by h*sy.
// The near (front) half of each ellipse is its LOWER half (larger screen-y).
// ---------------------------------------------------------------------------
...
function makeShellScene(rMax: number, hMax: number): ShellScene {
  ...
  // The outermost rim's vertical semi-axis bulges below the base and above the
  // top rim, so reserve 2*bOuter of vertical room for it.
...
/**
 * Front-visible lateral wall of an upright cylinder: bounded above by the near
 * (lower) half of the top rim, below by the near half of the base rim, and left/
 * right by the silhouette edges at cx +/- a.
 */
...
export function SolidOfRevolution({
  ...
  // Self-demo: sweep the representative slice/shell across [a, b] so the element
  // travels through the whole solid on its own.
  ...
  return (
    <WidgetFigure
      label={visual.label}
      // The slice readout (radius/height + ΔV formula + total V) is long and its
      // numbers change as the slice is dragged, so reserve three lines so it
      // never grows/shrinks and reflows the plot.
      captionLines={3}
      caption={caption}
      instruction={
        <MathText
...
/**
 * The faux-3D nest of cylindrical shells (shell method). Concentric translucent
 * shells share one vertical revolution axis; the draggable representative shell
 * is a highlighted hollow tube of radius x, height f(x) and thickness dx, with
 * dashed radius (x) and height (h) guides reinforcing dV = 2*pi*x*f(x)*dx.
 */
function ShellScene3D({
  ...
  // Concentric "growth-ring" shells across the domain (back-to-front = small to
  // large radius), each a thin cylinder whose top rim sits at height f(r).
  const ghosts = [];
  ...
  // Representative shell (a tube of thickness dx): exaggerate the wall to a
  // legible minimum so it always reads as a distinct 3D shell.
  ...
  return (
    ...
    >
      ...
      <g clipPath={`url(#${clipId})`}>
        ...
        {/* Nested cylindrical shells: concentric translucent "cans" whose walls
            grow taller toward the rim. Walls first (back-to-front), then crisp
            top rims, so the nesting stays legible. */}
        {ghosts.map((g, index) =>
          g.r >= hi ? null : (
            <path
              key={`wall-${index}`}
        ...
        {/* Outer wall + mouth of the whole solid (its 3D silhouette). Translucent
            so the nested shells inside remain visible through the near wall. */}
        <path
        ...
        {/* The representative cylindrical shell: a highlighted hollow tube of
            radius x, height f(x) and thickness Δx, drawn on top of the nest. */}
        <g>
